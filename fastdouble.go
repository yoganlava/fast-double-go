...
func computeFloat64(power int64, i uint64, negative bool, success *bool) float64 {
	...
	factorMantissa := mantissa64[power+325]

	exponent := (((152170 + 65536) * power) >> 16) + 1024 + 63
	lz := bits.LeadingZeros64(i)
	i <<= lz

	upper, lower := bits.Mul64(i, factorMantissa)

	if ((upper & 0x1FF) == 0x1FF) && (lower+i < lower) {
		factorMantissaLow := mantissa128[power+325]
		productMiddle2, productLow := bits.Mul64(i, factorMantissaLow)
		productMiddle1 := lower
		productHigh := upper
		productMiddle := productMiddle1 + productMiddle2
		if productMiddle < productMiddle1 {
			productHigh++
		}
		if (productMiddle+1 == 0) && ((productHigh & 0x1FF) == 0x1FF) && (productLow+i < productLow) {
			*success = false
			return 0
		}
		upper = productHigh
	...
	mantissa &= ^(uint64(1) << 52)
	realExponent := exponent - int64(lz)

	if realExponent < 1 || realExponent > 2046 {
		*success = false
		return 0
	}
	var negativeBit uint64
	if negative {
		negativeBit = 0
	}
	mantissa |= uint64(realExponent << 52)
	mantissa |= negativeBit << 63
	return *(*float64)(unsafe.Pointer(&mantissa))
}

func ParseNumber(p uintptr, outDouble *float64) error {
	foundMinus := (*(*byte)(unsafe.Pointer(p))) == '-'
	negative := false

	if foundMinus {
		p++
		negative = true
		if !isInteger((*(*byte)(unsafe.Pointer(p)))) {
			return errors.New("Negative sign must be followed by an integer")
		}
	}
	...
	if (*(*byte)(unsafe.Pointer(p))) == '0' {
		...
		digit := (*(*byte)(unsafe.Pointer(p))) - '0'
		i = uint64(digit)
		p++

		for isInteger((*(*byte)(unsafe.Pointer(p)))) {
			digit = (*(*byte)(unsafe.Pointer(p))) - '0'
			i = 10*i + uint64(digit)
			p++
		}
	...
	if (*(*byte)(unsafe.Pointer(p))) == '.' {
		...
		if isInteger((*(*byte)(unsafe.Pointer(p)))) {
			...
			return errors.New("Period must be followed by an integer")
		}
		...
		exponent = int64(firstAfterPeriod - p)
	}

	digitCount := int(p - startDigits - 1)

	if 'e' == (*(*byte)(unsafe.Pointer(p))) || 'E' == (*(*byte)(unsafe.Pointer(p))) {
		p++
		negExp := false
		if '-' == (*(*byte)(unsafe.Pointer(p))) {
			negExp = true
			p++
		} else if '+' == (*(*byte)(unsafe.Pointer(p))) {
			p++
		}
		if !isInteger((*(*byte)(unsafe.Pointer(p)))) {
			return errors.New("Exponent must be followed by an integer")
		}
		digit := (*(*byte)(unsafe.Pointer(p))) - '0'
		expNumber := int64(digit)
		p++
		if isInteger((*(*byte)(unsafe.Pointer(p)))) {
			digit = (*(*byte)(unsafe.Pointer(p))) - '0'
			expNumber = 10*expNumber + int64(digit)
			p++
		}
		if isInteger((*(*byte)(unsafe.Pointer(p)))) {
			digit = (*(*byte)(unsafe.Pointer(p))) - '0'
	...
	if digitCount >= 19 {
		...
		if digitCount >= 19 {
			// TODO !!!!!!!!
			// ! FALL BACK
			// ! return tuple (outDouble, err)
			return errors.New("Overflow :(")
		}
	}
	if exponent < -325 || exponent > 308 {
		return errors.New("Exponent exceeds range!")
	}
	success := true

	*outDouble = computeFloat64(exponent, i, negative, &success)
	if !success {
		return errors.New("Errored when computing float")
	}
	return nil
}
